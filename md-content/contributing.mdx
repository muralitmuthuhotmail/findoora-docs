...
To ensure code quality and consistency, Findoora Docs uses **git hooks** powered by [Husky](https://typicode.github.io/husky/) and [lint-staged](https://github.com/okonet/lint-staged):

- **Pre-commit hook**: Runs ESLint and Prettier on staged files, preventing commits with lint or formatting errors.
- **Pre-push hook**: Runs type checking and tests, blocking pushes if there are type errors or failing tests.
...
1. After cloning, run `pnpm dlx husky install` to set up hooks (if not already present).
2. Hooks will run automatically on commit and push.

> This ensures every contribution meets open source standards for code quality, formatting, and reliability.
