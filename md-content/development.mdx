...
export class AdvancedSearch {
  ...
  private applyFilters(results: Fuse.FuseResult<ContentPost>[], filters: SearchFilters): Fuse.FuseResult<ContentPost>[] {
    return results.filter(result => {
      if (filters.categories && !filters.categories.includes(result.item.category)) {
        return false;
      }
      
      if (filters.tags && !filters.tags.some(tag => result.item.tags?.includes(tag))) {
        return false;
      }
...
test.describe('Navigation', () => {
  ...
  test('should search for content', async ({ page }) => {
    ...
    // Click on result
    await page.click('text=Installation Guide');
